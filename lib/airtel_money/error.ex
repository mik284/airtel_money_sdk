defmodule AirtelMoney.Error do
  ...
  @drc_error_descriptions %{
    ...
    "DP00800001000" =>
      "Transaction is still processing and is in ambiguous state. Please do the transaction enquiry to fetch the transaction status.",
    ...
    "DP00900001014" =>
      "Transaction Timed Out. The transaction may be processed or failed due to time out. To know the exact status please do the transaction enquiry.",
    "DP00900001015" => "Transaction Not Found.",
    "DP00900001016" => "Forbidden: X-signature and payload did not match.",
    "DP00900001017" =>
      "Duplicate Transaction Id. To know the status of the actual transaction, please do transaction enquiry.",
  ...
  defp format_message(message, code, status) do
    formatted_message =
      cond do
        ...
        is_map(message) ->
          Map.get(message, "description") || Map.get(message, :description) || inspect(message)

        true ->
          "Unknown error (code: #{inspect(code)}, status: #{status})"
      end
